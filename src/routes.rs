...
pub async fn get_user_by_id(
    ...
    let user = web::block(move || {
        let mut conn = pool.get()?;
        db::find_user_by_uid(&mut conn, user_id.to_string())
    })
    .await?
...
pub async fn get_convo_by_id(
    ...
    let convos = web::block(move || {
        let mut conn = pool.get()?;
        db::get_convo_by_room_uid(&mut conn, room_id.to_string())
    })
    .await?
    .map_err(actix_web::error::ErrorInternalServerError)?;
    if convos.len() > 0  {
        ...
        let res = HttpResponse::NotFound().body(
            json!({
                "error": 404,
                "message": format!("No convo with room_id: {room_id}")
            })
            .to_string(),
        );
        Ok(res)
